...
const fetchJson = (url, body) => fetch(url, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
...
// 10% de chance de mandar zona errada p/ gerar alerta
const wrong = Math.random() < 0.1;
const zone = wrong ? randItem(zones.filter(z => !((plate==='PLT-1234'&&z==='Z1')||(plate==='PLT-9876'&&z==='Z2')||(plate==='PLT-5555'&&z==='Z3'))))
...
setInterval(tick, 1500); // a cada 1.5s
